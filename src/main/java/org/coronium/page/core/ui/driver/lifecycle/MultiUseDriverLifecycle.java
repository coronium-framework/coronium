...
public class MultiUseDriverLifecycle implements DriverLifecycle{
    ...
    /**
     * Create {@link Driver}s in parallel and addthem to the pool up to the
     * size specified.
    ...
    public void initDriverPool(Supplier<Driver> driverSupplier){
        if(driverPool != null){
            throw new IllegalStateException("initDriverPool called when its already initialised");
        }
    ...
    /**
     * If reuseBrowser is true, this will {@code deleteAllCookies} and
     * then re-add the {@link Driver} back to the pool,else it will call
     * {@code quit()} on the underlying {@link WebDriver}.
     */
    @Override
    ...
    /**
     * Drains the pool, call {@link WebDriver#quit()} on every
     * {@link Driver} remaining in the pool and sets the pool to
     * {@code null}.
     */
    @Override
    public void tearDownDriverPool(){
        ...
        driverPool = null; //allow re-initilisation
    }
}
